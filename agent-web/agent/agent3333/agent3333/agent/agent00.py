...
def aki_risk_predictor(patient_features: dict) -> dict:
    ...
    """
    aki_model=joblib.load(MODEL_DIR / "true_model.pkl")
    scaler = joblib.load(MODEL_DIR / "model.pklscaler.pkl")
...
def plot_sine_wave(patient_features: dict) -> dict:
    ...
    aki_model=joblib.load(MODEL_DIR / "model.pkl")
...
def aki_Is_serious(patient_features: dict) -> dict:
    ...
    """
    aki_model=joblib.load(MODEL_DIR / "true_model_2.pkl")
...
def operation_time_causal_effect(patient_features: dict, new_operation_time: float) -> dict:
    ...
    """
    bundle = joblib.load(
        r"E:\agent-web\agent\agent3333\agent3333\agent\model\Operation time.pkl"
    )
    model = bundle["model"]
    # 转 dataframe
...
def liquid_balance_causal_effect(patient_features: dict, new_liquid_balance: float) -> dict:
    ...
    """
    bundle = joblib.load(
        r"E:\agent-web\agent\agent3333\agent3333\agent\model\liquid balance.pkl"
    )
    model = bundle["model"]
    # 转 dataframe
    df = pd.DataFrame([patient_features])
...
def intraoperative_Partial_nephrectomy_causal_effect(patient_features: dict, new_intraoperative_Partial_nephrectomy: float) -> dict:
    ...
    """
    bundle = joblib.load(
        r"E:\agent-web\agent\agent3333\agent3333\agent\model\intraoperative Partial nephrectomy.pkl"
    )
    model = bundle["model"]
    # 转 dataframe
    df = pd.DataFrame([patient_features])
...
async def chat(file: UploadFile = File(None),   # 文件（可选）
    ...
    if not message:
        persistent_state["messages"].append(HumanMessage(content=' '))
    else:
